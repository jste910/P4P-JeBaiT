...
{
    vector<int8_t> weights(wrapper_constants_v3::DIGITCAPS_WEIGHT_COUNT);
    ifstream file(path, ios::binary);
    if (!file)
        throw runtime_error("Cannot open weight file: " + path);

    file.read(reinterpret_cast<char*>(weights.data()), wrapper_constants_v3::DIGITCAPS_WEIGHT_BYTES);
    if (file.gcount() != static_cast<streamsize>(wrapper_constants_v3::DIGITCAPS_WEIGHT_BYTES))
...
{
    if (argc < 7 || argc > 9) {
        cerr << "Usage: ./capsnet_full_v3.exe <xmodel> <xclbin> <images> "
             << "<fixed7_1_weights.bin> <num_images> <labels> [repetitions] [output_dir]\n";
        return 1;
    }
    ...
    vector<float> digitcaps_output(wrapper_constants_v3::DIGITCAPS_OUTPUT_COUNT);
