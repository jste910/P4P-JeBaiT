...
int main(int argc, char* argv[])
{
    ...
    const uint64_t repetitions = argc > 4 ? std::stoull(argv[4]) : 100;
    ...
    std::cout << std::fixed << std::setprecision(6)
              << "CALLS=" << repetitions << '\n'
              << "READ_TOTAL_MS=" << elapsedMs << '\n'
              << "READ_LATENCY_PER_CALL_MS=" << elapsedMs / repetitions << '\n';
