...
void dumpOutput(const fs::path& path, const float* data)
{
    ...
    for (std::size_t i = 0; i < outputSize; ++i) file << data[i] << '\n';
}
...
int main(int argc, char* argv[])
{
    ...
    const uint32_t imageCount = static_cast<uint32_t>(std::stoul(argv[4]));
    const fs::path outputDir = argv[5];
    const std::string inputFileName = argc > 6 ? argv[6] : "primary_squash_output.txt";
    const uint32_t repetitions = argc > 7 ? static_cast<uint32_t>(std::stoul(argv[7])) : 100;
    ...
    for (uint32_t repeat = 0; repeat < repetitions; ++repeat) {
        for (uint32_t i = 0; i < imageCount; ++i) {
            ...
            totalStageMs += std::chrono::duration<double, std::milli>(end - start).count();
    ...
    std::cout << std::fixed << std::setprecision(6)
              << "RUNS=" << runs << '\n'
              << "STAGE_ACTIVE_WINDOW_MS=" << totalStageMs << '\n'
              << "STAGE_LATENCY_PER_INFERENCE_MS=" << totalStageMs / runs << '\n';
