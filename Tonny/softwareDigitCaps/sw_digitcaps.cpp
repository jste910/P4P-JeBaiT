    ...
    std::size_t count)
{
    ...
    for (std::size_t i = 0; i < count; ++i) {
        if (!(file >> values[i]))
            throw std::runtime_error(
                "Not enough values in file: " + path.string());
    }
...
int main(int argc, char* argv[])
{
    if (argc < 5 || argc > 7) {
        std::cerr
            << "Usage: " << argv[0]
            << " <weights.txt> <input_dir> <image_count> <output_dir>"
    ...
    try {
        ...
        // -----------------------------------------------------
        // Load weights before measurement
        // -----------------------------------------------------
        auto weights =
            load_values(
                weights_path,
        ...
        for (uint32_t i = 0; i < WARMUP_RUNS; ++i) {
            float* input =
                inputs.data()
                + static_cast<std::size_t>(i % image_count)
                  * INPUT_SIZE;
        ...
        // -----------------------------------------------------
        // Measure only repeated software DigitCaps execution.
        // File I/O and output saving remain outside this window.
        // -----------------------------------------------------
        std::cout
            << "MEASUREMENT_START"
            << std::endl;
        ...
        const double total_software_ms =
            std::chrono::duration<
                double,
                std::milli>(
                    end - start)
        ...
        std::cout
            ...
            << "SOFTWARE_ACTIVE_WINDOW_MS="
            << total_software_ms
            << '\n'

            << "SOFTWARE_LATENCY_PER_INFERENCE_MS="
            << latency_per_inference_ms
            << '\n';

        // -----------------------------------------------------
        // Generate and save one final output per image after
        // measurement, so output copying and file I/O are not
        // included in the active software window.
        // -----------------------------------------------------
