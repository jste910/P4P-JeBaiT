...
#include "wrapper_constants_v3.h"
#include "xrt/xrt_bo.h"
#include "xrt/xrt_device.h"
#include "xrt/xrt_kernel.h"

static constexpr float FIXED_SCALE = 65536.0f;
static constexpr float DIGITCAPS_WEIGHT_SCALE = 64.0f; // 2^(7-1)

class Accel_Wrapper3{
public: 
    //constructors
    xrt :: device device;
    ...
    std::vector<int32_t> primary_squash_input_fixed;
    std::vector<int32_t> primary_squash_output_fixed;

    std::vector<int32_t> digitcaps_input_fixed;
    std::vector<int32_t> digitcaps_output_fixed;

    std::vector<int32_t> capsnet_length_input_fixed;
    std::vector<int32_t> capsnet_length_output_fixed;

    Accel_Wrapper3(const std::string& xclbin_path)
    {
        device = xrt :: device(0);
    ...
    {
        primary_squash_input_fixed.resize(wrapper_constants_v3::PRIMARY_SQUASH_INPUT_COUNT);
        primary_squash_output_fixed.resize(wrapper_constants_v3::PRIMARY_SQUASH_OUTPUT_COUNT);

        primary_squash_kernel = xrt::kernel(device, xclbin_uuid, "primary_squash_accel");
        primary_squash_input_bo = xrt::bo(device, wrapper_constants_v3::PRIMARY_SQUASH_INPUT_BYTES,primary_squash_kernel.group_id(0));
        primary_squash_output_bo = xrt::bo(device, wrapper_constants_v3::PRIMARY_SQUASH_OUTPUT_BYTES,primary_squash_kernel.group_id(1));
    ...
    void update_primary_squash_kernel(const float* primary_squash_input)
    {
        ...
        primary_squash_input_bo.write(primary_squash_input_fixed.data(), wrapper_constants_v3::PRIMARY_SQUASH_INPUT_BYTES,0);
        primary_squash_input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    }
    void run_primary_squash_kernel()
    {
        primary_squash_run = primary_squash_kernel(primary_squash_input_bo,primary_squash_output_bo);
        primary_squash_run.wait();
    }
    void read_primary_squash_kernel(float* primary_squash_output)
    {
        primary_squash_output_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
        primary_squash_output_bo.read(primary_squash_output_fixed.data(), wrapper_constants_v3::PRIMARY_SQUASH_OUTPUT_BYTES,0);
    ...
    void execute_primary_squash_kernel(float* primary_squash_output)
    {
        ...
        read_primary_squash_kernel(primary_squash_output);
    }
    ...
    void initialise_digitcaps_kernel(const float* digitcaps_weight)
    {
        ...
        std::vector<int8_t> digitcaps_weight_fixed(wrapper_constants_v3::DIGITCAPS_WEIGHT_COUNT);
        convert_float_to_fixed7_1(digitcaps_weight,digitcaps_weight_fixed.data(),digitcaps_weight_fixed.size());
        ...
        digitcaps_weight_bo.write(digitcaps_weight_fixed.data(),wrapper_constants_v3::DIGITCAPS_WEIGHT_BYTES,0);
    ...
    void update_digitcaps_kernel(const float* digitcaps_input)
    {
        ...
        digitcaps_input_bo.write(digitcaps_input_fixed.data(), wrapper_constants_v3::DIGITCAPS_INPUT_BYTES,0);
    ...
    void read_digitcaps_kernel(float* digitcaps_output)
    {
        ...
        digitcaps_output_bo.read(digitcaps_output_fixed.data(),wrapper_constants_v3::DIGITCAPS_OUTPUT_BYTES,0);
    ...
    {
        capsnet_length_input_fixed.resize(wrapper_constants_v3::CAPSNET_LENGTH_INPUT_COUNT);
        capsnet_length_output_fixed.resize(wrapper_constants_v3::CAPSNET_LENGTH_OUTPUT_COUNT);

        capsnet_length_kernel = xrt :: kernel(device,xclbin_uuid,"capsnet_length_accel");
        capsnet_length_input_bo = xrt :: bo(device, wrapper_constants_v3::CAPSNET_LENGTH_INPUT_BYTES,capsnet_length_kernel.group_id(0));
        capsnet_length_output_bo = xrt :: bo(device, wrapper_constants_v3::CAPSNET_LENGTH_OUTPUT_BYTES,capsnet_length_kernel.group_id(1));
    }
    ...
    void update_capsnet_length_kernel(const float* capsnet_length_input)
    {
        ...
        capsnet_length_input_bo.write(capsnet_length_input_fixed.data(), wrapper_constants_v3::CAPSNET_LENGTH_INPUT_BYTES,0);
        capsnet_length_input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    }

    void run_capsnet_length_kernel()
    {
        capsnet_length_run = capsnet_length_kernel(capsnet_length_input_bo,capsnet_length_output_bo);
        capsnet_length_run.wait();
    }

    void read_capsnet_length_kernel(float* capsnet_length_output)
    {
        capsnet_length_output_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
        capsnet_length_output_bo.read(capsnet_length_output_fixed.data(),wrapper_constants_v3::CAPSNET_LENGTH_OUTPUT_BYTES,0);
    ...
    {
        run_capsnet_length_kernel();
        read_capsnet_length_kernel(capsnet_length_output);
    }
    ...
    static void convert_float_to_fixed7_1(
        const float* input,
        int8_t* output,
        std::size_t size)
    {
