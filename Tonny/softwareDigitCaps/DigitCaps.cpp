...
void dynamic_routing(float *input, float *weights, float *prediction)
{
	...
	// Dynamic routing requires all initial logits b_ij to be zero.
	float *coupling_b = (float *)calloc(DIGIT_CAPS_NUM_DIGITS * DIGIT_CAPS_INPUT_CAPSULES, sizeof(float));
...
static void softmax(float *mat_b, float *mat_c)
{
	...
	{
		// Subtracting the largest logit avoids overflow in exp().
		float max_logit = mat_b[i];
		for (uint32_t j = 1; j < DIGIT_CAPS_NUM_DIGITS; ++j)
		{
			float logit = mat_b[i + j * DIGIT_CAPS_INPUT_CAPSULES];
			if (logit > max_logit)
			{
				max_logit = logit;
			}
		...
		{
			float entry = exp(mat_b[i + j * DIGIT_CAPS_INPUT_CAPSULES] - max_logit);
		...
		for (uint32_t j = 0; j < DIGIT_CAPS_NUM_DIGITS; ++j)
		{
			...
			mat_c[i + j * DIGIT_CAPS_INPUT_CAPSULES] /= sum;
		}
	}
}
