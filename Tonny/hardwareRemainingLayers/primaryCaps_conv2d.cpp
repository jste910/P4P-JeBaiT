...
constexpr std::size_t INPUT_SIZE = 20 * 20 * 256;
constexpr std::size_t OUTPUT_SIZE = 6 * 6 * 256;
constexpr uint32_t WARMUP_RUNS = 10;

static fs::path get_input_path(
    ...
    const std::string& filename)
{
    ...
    const fs::path flat_path =
        directory / (image_name + ".txt");

    if (fs::exists(flat_path))
        return flat_path;

    return directory / image_name / filename;
}
...
static void load_quantised_input(
    const fs::path& path,
    int8_t* destination,
    ...
    int fix_point)
{
    ...
    for (std::size_t i = 0; i < element_count; ++i) {
        ...
        if (!(file >> value))
            throw std::runtime_error(
                "Incorrect input size in: " + path.string());

        destination[i] = quantise(value, fix_point);
    }
}

static void dump_dequantised_output(
    const fs::path& path,
    const int8_t* data,
    std::size_t element_count,
    int fix_point)
{
    ...
    const float scale = std::ldexp(1.0f, -fix_point);

    file << std::setprecision(10);

    for (std::size_t i = 0; i < element_count; ++i) {
        file << static_cast<float>(data[i]) * scale << '\n';
    }
}
...
int main(int argc, char* argv[])
{
    if (argc < 5 || argc > 7) {
        std::cerr
            << "Usage: " << argv[0]
            << " <xmodel> <input_dir> <image_count> <output_dir> "
    ...
    try {
        ...
        std::vector<int8_t> saved_output(
            static_cast<std::size_t>(image_count) * output_size);

        // Use the original INT8 tensors—no FLOAT32 wrappers.
        auto input_buffer =
        ...
        std::cout
            ...
            << "INPUT_FIX_POINT=" << input_fix_point << '\n'
            << "OUTPUT_FIX_POINT=" << output_fix_point << '\n'
        ...
        // Dequantisation and file writing happen after measurement.
        fs::create_directories(output_dir);

        for (uint32_t i = 0; i < image_count; ++i) {
            dump_dequantised_output(
                output_dir /
                    ("img" + std::to_string(i) + ".txt"),
                saved_output.data() +
