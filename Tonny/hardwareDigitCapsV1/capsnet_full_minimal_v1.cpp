...
static vector<int32_t> load_weights(const string& path)
{
    vector<int32_t> weights(wrapper_constants_v1::DIGITCAPS_WEIGHT_COUNT);
    ifstream file(path, ios::binary);
    if (!file)
        throw runtime_error("Cannot open weight file: " + path);

    file.read(reinterpret_cast<char*>(weights.data()), wrapper_constants_v1::DIGITCAPS_WEIGHT_BYTES);
    if (file.gcount() != static_cast<streamsize>(wrapper_constants_v1::DIGITCAPS_WEIGHT_BYTES))
...
{
    if (argc < 7 || argc > 9) {
        cerr << "Usage: ./capsnet_full_v1.exe <xmodel> <xclbin> <images> "
             << "<fixed_weights.bin> <num_images> <labels> [repetitions] [output_dir]\n";
        return 1;
    }
    ...
    vector<float> squash_output(wrapper_constants_v1::PRIMARY_SQUASH_OUTPUT_COUNT);
    vector<float> digitcaps_output(wrapper_constants_v1::DIGITCAPS_OUTPUT_COUNT);
    vector<float> length_output(wrapper_constants_v1::CAPSNET_LENGTH_OUTPUT_COUNT);
