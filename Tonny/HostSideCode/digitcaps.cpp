...
constexpr std::size_t INPUT_SIZE =
    wrapper_constants::DIGITCAPS_INPUT_COUNT;

constexpr std::size_t WEIGHT_SIZE =
    wrapper_constants::DIGITCAPS_WEIGHT_COUNT;

constexpr std::size_t OUTPUT_SIZE =
    wrapper_constants::DIGITCAPS_OUTPUT_COUNT;
...
int main(int argc, char* argv[])
{
    ...
    const std::string input_filename =
        argc > 6 ? argv[6] : "primary_squash_output.txt";
    ...
    accelerator.initialise_digitcaps_kernel(weights.data());

    std::vector<float> output(OUTPUT_SIZE);

    for (uint32_t i = 0; i < WARMUP_RUNS; ++i) {
        accelerator.update_digitcaps_kernel(inputs.data());
        accelerator.execute_digitcaps_kernel(output.data());
    }
