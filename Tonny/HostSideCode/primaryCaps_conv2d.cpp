...
constexpr int INPUT_SIZE = 20 * 20 * 256;
constexpr int OUTPUT_SIZE = 6 * 6 * 256;
...
int main(int argc, char* argv[])
{
    ...
    const std::string input_filename =
        argc > 5 ? argv[5] : "convolutional_output.txt";
    ...
    std::vector<float> images(
        static_cast<size_t>(image_count) * INPUT_SIZE);
    ...
    for (uint32_t i = 0; i < image_count; ++i) {
        ...
        std::copy(
            ...
            values.end(),
            images.begin() +
                static_cast<size_t>(i) * INPUT_SIZE);
    }
