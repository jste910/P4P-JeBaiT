...
constexpr std::size_t INPUT_SIZE =
    wrapper_constants::CAPSNET_LENGTH_INPUT_COUNT;

constexpr std::size_t OUTPUT_SIZE =
    wrapper_constants::CAPSNET_LENGTH_OUTPUT_COUNT;
...
int main(int argc, char* argv[])
{
    ...
    const std::string input_filename =
        argc > 5 ? argv[5] : "digitcaps_output.txt";
    ...
    for (uint32_t i = 0; i < WARMUP_RUNS; ++i) {
        ...
        accelerator.execute_capsnet_length_kernel(output.data());
    }
