...
#include "wrapper_constants_v2.h"
#include "xrt/xrt_bo.h"
#include "xrt/xrt_device.h"
#include "xrt/xrt_kernel.h"

static constexpr float FIXED_SCALE = 65536.0f;
static constexpr float DIGITCAPS_WEIGHT_SCALE = 0.004576775032704271f;
class Accel_Wrapper2{
public: 
    //constructors
    xrt :: device device;
    ...
    Accel_Wrapper2(const std::string& xclbin_path)
    {
        device = xrt :: device(0);
    ...
    void initialise_primary_squash_kernel()
    {
        ...
        primary_squash_input_bo = xrt::bo(device, wrapper_constants::PRIMARY_SQUASH_INPUT_BYTES,primary_squash_kernel.group_id(0));
        primary_squash_output_bo = xrt::bo(device, wrapper_constants::PRIMARY_SQUASH_OUTPUT_BYTES,primary_squash_kernel.group_id(1));
        
    }
    void update_primary_squash_kernel(const float* primary_squash_input)
    {
        std :: vector<int32_t> primary_squash_input_fixed (wrapper_constants::PRIMARY_SQUASH_INPUT_COUNT);
        convert_float_to_fixed32_16(primary_squash_input,primary_squash_input_fixed.data(),primary_squash_input_fixed.size());

        primary_squash_input_bo.write(primary_squash_input_fixed.data(), wrapper_constants::PRIMARY_SQUASH_INPUT_BYTES,0);
        primary_squash_input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    }

    void execute_primary_squash_kernel(float* primary_squash_output)
    {
        std :: vector<int32_t> primary_squash_output_fixed (wrapper_constants::PRIMARY_SQUASH_OUTPUT_COUNT);
        ...
        primary_squash_output_bo.read(primary_squash_output_fixed.data(), wrapper_constants::PRIMARY_SQUASH_OUTPUT_BYTES,0);
    ...
    {
        std::vector<int8_t> digitcaps_weight_int8( wrapper_constants::DIGITCAPS_WEIGHT_COUNT);
        convert_float_to_int8_weights( digitcaps_weight, digitcaps_weight_int8.data(),digitcaps_weight_int8.size());

        digitcaps_kernel = xrt::kernel(device, xclbin_uuid, "digitcaps_accel");
        digitcaps_input_bo = xrt::bo(device, wrapper_constants::DIGITCAPS_INPUT_BYTES,digitcaps_kernel.group_id(0));
        digitcaps_weight_bo = xrt::bo(device, wrapper_constants::DIGITCAPS_WEIGHT_BYTES,digitcaps_kernel.group_id(1));
        digitcaps_output_bo = xrt::bo(device, wrapper_constants::DIGITCAPS_OUTPUT_BYTES,digitcaps_kernel.group_id(2));
        
        digitcaps_weight_bo.write(digitcaps_weight_int8.data(),wrapper_constants::DIGITCAPS_WEIGHT_BYTES,0);
    ...
    void initialise_digitcaps_kernel_fixed(const int8_t* digitcaps_weight_fixed)
    {
        ...
        digitcaps_weight_bo.write(digitcaps_weight_fixed,wrapper_constants::DIGITCAPS_WEIGHT_BYTES,0);
        digitcaps_weight_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    }

    void update_digitcaps_kernel(const float* digitcaps_input)
    {
        std::vector<int32_t> digitcaps_input_fixed(wrapper_constants::DIGITCAPS_INPUT_COUNT);
        convert_float_to_fixed32_16(digitcaps_input,digitcaps_input_fixed.data(),digitcaps_input_fixed.size());

        digitcaps_input_bo.write(digitcaps_input_fixed.data(), wrapper_constants::DIGITCAPS_INPUT_BYTES,0);
    ...
    {
        std::vector<int32_t> digitcaps_output_fixed(wrapper_constants::DIGITCAPS_OUTPUT_COUNT);

        digitcaps_output_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
        digitcaps_output_bo.read(digitcaps_output_fixed.data(),wrapper_constants::DIGITCAPS_OUTPUT_BYTES,0);
    ...
    void initialise_capsnet_length_kernel()
    {
        ...
        capsnet_length_input_bo = xrt :: bo(device, wrapper_constants::CAPSNET_LENGTH_INPUT_BYTES,capsnet_length_kernel.group_id(0));
        capsnet_length_output_bo = xrt :: bo(device, wrapper_constants::CAPSNET_LENGTH_OUTPUT_BYTES,capsnet_length_kernel.group_id(1));
    }

    void update_capsnet_length_kernel(const float* capsnet_length_input)
    {
        std::vector<int32_t> capsnet_length_input_fixed(wrapper_constants::CAPSNET_LENGTH_INPUT_COUNT);
        convert_float_to_fixed32_16(capsnet_length_input,capsnet_length_input_fixed.data(),capsnet_length_input_fixed.size());

        capsnet_length_input_bo.write(capsnet_length_input_fixed.data(), wrapper_constants::CAPSNET_LENGTH_INPUT_BYTES,0);
        capsnet_length_input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);
    }

    void execute_capsnet_length_kernel(float* capsnet_length_output)
    {
        std::vector<int32_t> capsnet_length_output_fixed(wrapper_constants::CAPSNET_LENGTH_OUTPUT_COUNT);
        ...
        capsnet_length_output_bo.read(capsnet_length_output_fixed.data(),wrapper_constants::CAPSNET_LENGTH_OUTPUT_BYTES,0);
    ...
    {
        int q = static_cast<int>(std::llround(x / DIGITCAPS_WEIGHT_SCALE));
