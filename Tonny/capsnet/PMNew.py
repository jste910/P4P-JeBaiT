...
# PC used for result storage.
PC_IP_ADDRESS = "192.168.9.1"
PC_USER = "tonny"
PC_DESTINATION = "/home/tonny/UV_Result/"
PC_PASSWORD = "1"
...
def offload(path, file=False):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Cannot offload missing path: {path}")

    if not pingHost(PC_IP_ADDRESS):
        raise ConnectionError(f"PC is unreachable at {PC_IP_ADDRESS}")
    ...
    command.extend([
        path,
        f"{PC_USER}@{PC_IP_ADDRESS}:{PC_DESTINATION}",
    ])
    ...
    match = child.expect([r"(?i)password:", pexpect.EOF])
    if match == 0:
        if PC_PASSWORD is None:
            PC_PASSWORD = getpass(
                f"Password for {PC_USER}@{PC_IP_ADDRESS}: "
            )
    ...
    if child.exitstatus != 0:
        raise RuntimeError(
            f"scp failed for {path} with exit status {child.exitstatus}"
        )

    print(f"Copied successfully: {path}")
...
def seperatedLoop(cwd, img, step, iter, voltingOrder):
    ...
    for dontuseme in range(iter):
        ...
        digitinferencerun = f"{RUNEXE} {XCLBIN} {ALT_WEIGHTS_PATH} {squashfolder}_{volt:.2f}V {img} {digitcapsfolder}_{volt:.2f}V"
        ...
        print(f'Voltage restored to: {NOMINAL_VOLTAGE:.2f}V')
        ...
        offload(f"{digitcapsfolder}_{volt:.2f}V",file=False)
...
def main():
    ...
    # Upload the completed rail-monitoring log.
    try:
        offload("./me.csv", file=True)
    except Exception as e:
        print(f"Could not upload me.csv; the local file was retained: {e}")
