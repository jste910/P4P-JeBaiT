...
measurement_energy_j = 0.0
measurement_previous_time_ns = None
measurement_previous_power_w = None
measurement_power_sum = 0.0
measurement_power_count = 0
...
def runCommand(cmd, cwd, logFile=None):
    result = subprocess.run(cmd, shell=True, cwd=cwd, capture_output=True, text=True)
    output = result.stdout + result.stderr
    print(output)  # keep showing it live in the terminal, same as before
...
def readAll(bus, voltageLocation, currentLocation, powerLogFile=None):
    ...
    if alt is not None and alt2 is not None:
        ...
        current = alt2 / 4096
        power = voltage * current
        print(f"Power: {voltage:.2f}V x {current:.2f}A = {power:.2f}W")
        # print(f"Power: {alt/4096:.2f}V ({alt}) x {alt2/4096:.2f}A ({alt2})= {(alt/4096)*(alt2/4096):.2f}W") # debug
        if powerLogFile is not None:
            fileExists = os.path.isfile(powerLogFile)
            with open(powerLogFile, "a", newline="") as f:
                writer = csv.writer(f)
                if not fileExists:
                    writer.writerow(["timestamp", "voltage_V", "current_A", "power_W"])
                writer.writerow([datetime.now().isoformat(), f"{voltage:.4f}", f"{current:.4f}", f"{power:.4f}"])

def getReadingsBus(busNumber, safe = True, powerLogFile=None):
    # safe = True means that we are threading and safe = False means we are not
    bus = smbus2.SMBus(busNumber)
    if not safe:
        readAll(bus, 0x8B, 0x8C, powerLogFile)
...
def undervoltingLoop(initialvoltage, cwd, cmd, iter, step, outputDir=None):
    volt = initialvoltage
    for i in range(iter):
        print("==============================")
        ...
        if outputDir is not None:
            logFile = os.path.join(outputDir, f"inference_step{i:02d}_{volt:.2f}V.txt")
        runCommand(cmd, cwd, logFile)
        volt -= step
...
def main():
    ...
    # Where this run's results (power log + per-step inference output) get saved
    RESULTS_DIR = os.path.join(HOME_PATH, "results", datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(RESULTS_DIR, exist_ok=True)
    POWER_LOG_FILE = os.path.join(RESULTS_DIR, "power_log.csv")
    ...
    if isThreaded:
        monitorThread = threading.Thread(target=getReadingsBus, args=(4, True, POWER_LOG_FILE), daemon=True)
        shellThread = threading.Thread(target=undervoltingLoop, args=(NOMINAL_VOLTAGE, cwd, cmd, ITER, STEP, RESULTS_DIR), daemon=True)
